...
class MemoryCachingResponder(delegate: Responder, cacheSizeInBytes: Long = 100000000, maxFileSizeInBytes: Long = 1000000) extends LoadingCacheResponder(delegate) {
  protected val cache: LoadingCache[ContentRequest, Option[Response]] = CacheBuilder.newBuilder().maximumSize(cacheSizeInBytes).build(loader)

  private def loader = new CacheLoader[ContentRequest, Option[Response]]() {
    def load(request: ContentRequest): Option[Response] = {
